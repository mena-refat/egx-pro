...
export default function UserDetailPage() {
  ...
  if (!user) return (
    <div className="text-center py-20">
      ...
      <button onClick={() => nav('/users')} className="mt-3 text-sm text-emerald-400 hover:underline">← Back</button>
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Status',      value: <span className={user.isDeleted ? 'text-red-400 text-sm' : 'text-emerald-400 text-sm'}>{user.isDeleted ? 'Deactivated' : 'Active'}</span> },
          { label: 'AI Uses',     value: <span className="text-white text-sm">{user.aiAnalysisUsedThisMonth}</span> },
          { label: 'Verified',    value: <span className={user.isEmailVerified ? 'text-emerald-400 text-sm' : 'text-slate-500 text-sm'}>{user.isEmailVerified ? 'Yes' : 'No'}</span> },
          { label: 'Joined',      value: <span className="text-slate-300 text-sm">{new Date(user.createdAt).toLocaleDateString()}</span> },
          { label: 'Last Login',  value: <span className="text-slate-300 text-sm">{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleDateString() : '—'}</span> },
          { label: 'Portfolio',   value: <span className="text-white text-sm">{user._count?.portfolios ?? 0}</span> },
          { label: 'Predictions', value: <span className="text-white text-sm">{user._count?.predictions ?? 0}</span> },
        ].map((item) => (
      ...
      <Modal open={planModal} onClose={() => setPlanModal(false)} title="Change Plan" width="max-w-sm">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs text-slate-400 block mb-1.5">Expires At (optional)</label>
            <input type="datetime-local" value={planDate} onChange={(e) => setPlanDate(e.target.value)}
              className="w-full px-3 py-2 text-sm bg-[#0d0d14] border border-white/[0.08] rounded-lg text-white focus:outline-none focus:border-emerald-500/50" />
          </div>
          <div className="flex gap-3 justify-end pt-2">
            <button onClick={() => setPlanModal(false)} className="px-4 py-2 text-sm text-slate-400 hover:text-slate-200 transition-colors">Cancel</button>

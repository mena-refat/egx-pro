...
export function PortfolioPerformanceCard({
  totalCost, totalGainLoss, totalGainLossPercent,
  cardBackground, borderColor, textColor, mutedTextColor,
}: Props) {
  const isPositive = totalGainLoss > 0;
  const isNegative = totalGainLoss < 0;
  const accent = isPositive ? '#4ade80' : isNegative ? '#f87171' : mutedTextColor;

  return (
    <View style={{ backgroundColor: cardBackground, borderColor, borderWidth: 1, borderRadius: 16, padding: 16, gap: 14 }}>
      <Text style={{ color: textColor, fontSize: 16, fontWeight: '700' }}>أداء المحفظة</Text>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <View style={{ gap: 4 }}>
          <Text style={{ color: mutedTextColor, fontSize: 12 }}>رأس المال</Text>
        ...
        <View style={{ gap: 4, alignItems: 'flex-end' }}>
          <Text style={{ color: mutedTextColor, fontSize: 12 }}>الربح/الخسارة</Text>
      ...
      <View style={{ height: 8, backgroundColor: '#8c959f18', borderRadius: 999, overflow: 'hidden' }}>
        <View style={{
          height: 8,

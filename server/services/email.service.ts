...
export const EmailService = {
  ...
  async sendVerificationCode(email: string, code: string): Promise<void> {
    ...
    await resend.emails.send({
      from: FROM,
      to: email,
      subject: 'كود التحقق من البريد الإلكتروني — EGX Pro',
      html: `
        <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #f9f9f9; border-radius: 12px;">
          <h2 style="color: #7c3aed; margin-bottom: 8px;">EGX Pro</h2>
    ...
    await resend.emails.send({
      ...
      html: `
        <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
          ...
          <p style="color: #333; font-size: 16px;">مرحباً بك في EGX Pro — منصتك لتحليل البورصة المصرية.</p>
  ...
  async sendPasswordChanged(email: string): Promise<void> {
    ...
    await resend.emails.send({
      from: FROM,
      to: email,
      subject: 'تم تغيير كلمة المرور — EGX Pro',
      html: `
        <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
          <h2 style="color: #dc2626;">⚠️ تنبيه أمني</h2>
          <p style="color: #333; font-size: 16px;">تم تغيير كلمة المرور لحسابك على EGX Pro.</p>

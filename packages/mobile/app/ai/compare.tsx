  ...
  return (
    ...
    >
      ...
      <View style={tw('items-center gap-2')}>
        <Text style={[{ color: colors.text }, tw('text-base font-bold')]}>{label}</Text>
        {typeof side.score === 'number' && (
          ...
          >
            <Text style={[{ color: scoreColor(score) }, tw('text-base font-bold')]}>{score}</Text>
          </View>
        )}
        {verdict ? (
      ...
      {str(side.fundamental) ? (
        <Text style={[{ color: colors.textSub }, tw('text-xs leading-5')]} numberOfLines={3}>
          {str(side.fundamental)}
        </Text>
      ) : null}
      <View style={tw('gap-1')}>
        {side.strengths?.slice(0, 2).map((s, i) => {
          ...
          return (
            <View key={i} style={tw('flex-row gap-1.5 items-start')}>
    ...
    <ScreenWrapper padded={false}>
      ...
      >
        ...
        <Pressable
          ...
          style={[
            tw('bg-blue-500 rounded-xl py-3.5 items-center'),
            { opacity: loading || !t1.trim() || !t2.trim() ? 0.5 : 1 },
          ]}
        >
          {loading ? (
        ...
        {result && !loading && (
          <View style={tw('gap-4')}>
            ...
            {result.disclaimer ? (
              <Text style={[{ color: colors.textMuted }, tw('text-xs text-center leading-5 px-2')]}>
                ⚖️ {result.disclaimer}
              </Text>
            ) : null}
          </View>

...
router.post('/', authenticate, watchlistAddLimiter, idempotencyMiddleware, validate(addWatchlistBodySchema, 'body'), WatchlistController.add);
router.patch('/:ticker', authenticate, idempotencyMiddleware, validate(tickerParamSchema, 'params'), validate(updateWatchlistBodySchema, 'body'), WatchlistController.update);
router.post('/check-targets', authenticate, idempotencyMiddleware, validate(checkTargetsBodySchema, 'body'), WatchlistController.checkTargets);
router.delete('/:ticker', authenticate, idempotencyMiddleware, validate(tickerParamSchema, 'params'), WatchlistController.remove);
